  ...
  if (addresses.length === 0) {
    return (
      <main className="mx-auto w-full max-w-lg flex-1 px-6 py-16 text-center">
        <h1 className="text-2xl font-semibold">Add a delivery address</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          You need at least one address on file before checking out.
        </p>
        <Link
          href="/account/addresses/new"
          className="mt-6 inline-block rounded-md bg-primary px-6 py-2.5 text-sm font-medium text-primary-foreground"
        >
          Add address
        </Link>
      </main>
